...
from transactions.services.interfaces.employee_by_admin_service import EmployeeByAdminService
...
class DefaultEmployeeByAdminService(EmployeeByAdminService):
    ...
    def _get_group(self):
        group = Group.objects.filter(name=self.group_name)
        if not group.exists():
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            ...
            employee.save()
            self._get_group().user_set.add(employee)
